if (typeof window !== "undefined") {
    ...
    const CIRCUIT_ALT_PROBABILITY = 0.15; // 15% chance of orange circuit
    ...
    */
    function init() {
        const canvas1 = setupBaseCanvas("c");
        ...
        drawStaticBackground(ctx1, width, height, dots.canvas, circuits.canvas);

        const ctx2 = createOverlayCanvas(width, height);
        startLoop(ctx2, particles);
    }
    ...
    */
    ...
    window.addEventListener('resize', () => {
        ...
        resizeTimeout = setTimeout(() => {
            // Remove old animated canvas
            document.querySelectorAll('canvas:not(#c)').forEach(c => c.remove());

            // Reset static canvas
            const staticCanvas = document.getElementById('c');
